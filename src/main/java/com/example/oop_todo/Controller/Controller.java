...
public class Controller {
    ...
    // Get the current time when launching the application
    ...
    public void initialize(Stage stage) throws IOException {
        ...
        // set the default to dark mode at night if current time is 8pm onward first run the app
    ...
    private ArrayList<Task> deleteTrashDataFromFile(ArrayList<Task> tasks) {
        ArrayList<Task> tasksInMainController = TaskList.tasks;
        if (!tasksInMainController.isEmpty()){
            for (Task taskObj: tasksInMainController){
                // write again the text file by excluding the trashed tasks
                if (!taskObj.getStatus().equals("trashed")){
                    tasksInMainController.add(taskObj);
                }
            }
    ...
    private void switchMainPageTheme(){
        ...
        if (!taskControllers.isEmpty()){ // set the theme of the task bar custom control
    ...
    public void displayTasks(String currPage) throws IOException {
        ...
        if (currentTime.getHour() >= 1) {
            System.out.println("Now is after 1am");
            loadTrash();
            saveTrash();
        }
